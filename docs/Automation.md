...
* `holiday_region` / `HolidayRegion`  (string): Skip automation if there is a formal, observed holiday for this region.
...
* `schedule_times_of_day` (string[]): Times of day to run in HH:MM format (24-hour). Required for `custom_schedule` triggers. Optional for `daily` triggers - if not set, runs at midnight UTC.
