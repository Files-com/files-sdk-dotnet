...
* `event_type` / `EventType`  (string): Type of SSO event being recorded.
...
* `username` / `Username`  (string): Username on Files.com for the SSO login attempt.
* `idp_uid` / `IdpUid`  (string): Identity Provider UID for the SSO login attempt.
* `provider` / `Provider`  (string): SSO provider for the SSO login attempt.
* `provider_label` / `ProviderLabel`  (string): SSO provider label for the SSO login attempt.
* `ip` / `Ip`  (string): IP address for the SSO login attempt.
* `region` / `Region`  (string): Region for the SSO login attempt.
...
```
Task<FilesList<SsoEvent>> SsoEvent.List(
...
* `id` (Nullable<Int64>): Required - Sso Event ID.
