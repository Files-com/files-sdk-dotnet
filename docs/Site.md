...
* `allow_user_level_2fa_override` / `AllowUserLevel2faOverride`  (bool): Allow the site-wide two-factor authentication requirement to be overriden on a per-user-basis?
* `allow_user_level_allowed_ip_override` / `AllowUserLevelAllowedIpOverride`  (bool): Allow the site-wide allowed IP restriction to be overriden on a per-user-basis?
* `allow_user_level_ssl_override` / `AllowUserLevelSslOverride`  (bool): Allow the site-wide FTP SSL requirement to be overriden on a per-user-basis?
...
* `bundle_send_one_time_password_to_recipient_at_registration` / `BundleSendOneTimePasswordToRecipientAtRegistration`  (bool): If true, new Share Links must send a one-time password to the recipient when they register. Requires bundle_require_share_recipient and cannot be enabled with bundle_password_required.
...
* `managed_site_settings` / `ManagedSiteSettings`  (object): List of site settings managed by the parent site
...
* `next_billing_amount` / `NextBillingAmount`  (decimal): Next billing amount
...
* `active_sftp_host_key_ids` / `ActiveSftpHostKeyIds`  (Nullable<Int64>[]): Ids of the selected custom SFTP Host Keys
...
* `email_footer_custom_text` / `EmailFooterCustomText`  (string): Custom footer text for system-generated emails (as Markdown). Supports standard strftime date/time patterns like %Y (4-digit year), %m (month), %d (day).
...
* `active_sftp_host_key_ids` (Nullable<Int64>[]): Ids of the selected custom SFTP Host Keys
* `protocol_access_groups_only` (bool): If true, protocol access permissions on users will be ignored, and only protocol access permissions set on Groups will be honored.  Make sure that your current user is a member of a group with API permission when changing this value to avoid locking yourself out of your site.
...
* `redirect_old_subdomain` (bool): If true, and if changing the site subdomain, then create a redirect from the previous Files.com subdomain to the new Files.com subdomain.
