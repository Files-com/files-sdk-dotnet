...
* `transformation_credits_usage` / `TransformationCreditsUsage`  (Nullable<Int64>): Transformation and AI credits used on this day. Note: initially this tracks GPG encrypt, decrypt, and recrypt usage.
...
* `usage_by_top_level_dir` / `UsageByTopLevelDir`  (UsageByTopLevelDir[]): Usage broken down by each top-level folder
